...
orderRouter.post("/login", async function (req, res, next) {
  try {
    // application/json 설정을 프론트에서 안 하면, body가 비어 있게 됨.
    if (is.emptyObject(req.body)) {
      throw new Error(
...
// 로그아웃 api
orderRouter.post("/logout", loginRequired, async (req, res, next) => {
...
orderRouter.get("/userlist", async function (req, res, next) {
  try {
    // 전체 사용자 목록을 얻음
